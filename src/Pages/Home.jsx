import React from 'react';
import { Link } from 'react-router-dom';
import Plagiarism_Checker from '../Assets/Image/plagiarism_checker.png';

const TextAnalysisTools = [
    {
        id: 1,
        name: 'Plagiarism Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Article Rewriter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Grammar Check',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Word Counter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Spell Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Paraphrasing Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'MD5 Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Uppercase To Lowercase',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Word Combiner',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Image To Text Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Translate English To English',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Text To Image',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Text To Speech',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'JPG To Word',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Online Text Editor',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Reverse Text Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    }
]

const DesignStudioTools = [
    {
        id: 1,
        name: 'Logo Maker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Resume Builder',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Flyer Maker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Poster Maker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Invitation Maker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Business Card Maker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    }
]

const ImagesEditingTools = [
    {
        id: 1,
        name: 'Reverse Image Search',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Resume Builder',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Favicon Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Image Resizer',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Crop Image',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Convert To JPG',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'RGB To Hex',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'PNG To JPG',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'JPG To PNG',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    }
]

const KeywordsTools = [
    {
        id: 1,
        name: 'Keyword Position',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Keywords Density Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Keywords Suggestions Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Keyword Research Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Keyword Competition Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Related Keywords Finder',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Long Tail Keyword Suggestion Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'keywords Rich Domains Suggestions Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'SEO Keyword Competition Analysis',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Live Keyword Analyzer',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Keyword Overview Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Keyword Difficulty Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Paid Keyword Finder',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    }
]

const BackLinkTools = [
    {
        id: 1,
        name: 'Backlink Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Backlink Maker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Website Link Count Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Website Broken Link Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Link Price Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Reciprocal Link Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Website Link Analyzer Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Broken Backlink Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Valuable Backlink Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Backlinks Competitors',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    },
    {
        id: 1,
        name: 'Anchor Text Distribution',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: "No-Images",
    }
]

const WebsiteManagementTools = [
    {
        id: 1,
        name: 'Website SEO Score Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
    id: 1,
        name: 'Google Pagerank Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Online Ping Website Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Page Speed Test',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Website Page Size Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Website Page Snooper',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Website Hit Counter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'XML Sitemap Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'URL Rewriting Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'What is My Screen Resolution',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'URL Encoder Decoder',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Adsense Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Open Graph Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image'
    },
    {    
        id: 1,
        name: 'Open Graph Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'QR Code Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Htaccess Redirect Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Get HTTP Headers',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Twitter Card Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Internet Speed Test',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Wordpress Theme Detector',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Instant Search Suggestions Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Online Virus Scan',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Website Screenshot Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Secure Email',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Mobile Friendly Test',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Video Downloader',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Facebook Video Downloader',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Soundcloud Downloader',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Vimeo Video Downloader',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Instagram Video Downloader',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Dailymotion Video Downloader',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Minify CSS',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Minify HTML',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Minify JS',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Robots.txt Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'URL Shortener',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'Website Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'URL Opener',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'PHP Formatter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'HTML Formatter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'HTML Editor',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'HTML Viewer',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'XML Formatter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {    
        id: 1,
        name: 'XML Beautifier',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const WebsiteTrackingTools = [
    {
        id:1,
        name: 'Link Tracker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Reverse IP Lookup',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Check Server Status',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Class C IP Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Code To Text Ratio Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Alexa Rank Comparison',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Page Comparison',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Spider Simulator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Comparison Search',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Google Cache Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Whois Lookup',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Mozrank Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Page Authority Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Google Index Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Alexa Rank Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Redirect Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Similar Site Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Cloaking Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Google Malware Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Find Facebook ID',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Check GZIP compression',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'SSL Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Pokemon Go Server Status Finder',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Find Blog Sites',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'GEO IP Locator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Apps Rank Tracking Tool',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'What Is My Browser',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Check Social Status',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const ProxyTools = [
    {
        id:1,
        name: 'What is My IP',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'IP Location',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Free Daily Proxy List',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const DomainsTools = [
    {
        id:1,
        name: 'Domain Age Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Domain Authority Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Domain IP Lookup',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Domain Hosting Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Find DNS records',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Domain Name Search',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Domain To IP',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Check Blacklist IP',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Find Expired Domains',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Bulk Domain Rating Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Index Pages Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const MetaTagsTools = [
    {
        id:1,
        name: 'Meta Tags Analyzer',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Meta Tag Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const PasswordManagementTools = [
    {
        id:1,
        name: 'Password Encryption Utility',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Password Strength Checker',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Password Generator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const OnlinePDFTools = [
    {
        id:1,
        name: 'Merge PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Rotate PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Unlock PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Lock PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Watermark',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'PDF To Word',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Word To PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'PDF To JPG',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'JPG To PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'PowerPoint To PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Compress PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Excel To PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Split PDF',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const DevelopmentTools = [
    {
        id:1,
        name: 'JSON Viewer',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'JSON Formatter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'JSON Validator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'JSON Editor',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'JSON To XML',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'JSON Beautifier',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const UnitConverterTools = [
    {
        id:1,
        name: 'Unit Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Time Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Power Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Speed Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Volume Conversion',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Length Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Voltage Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Area Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Weight Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Byte Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Temperature Conversion',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Torque Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Pressure Conversion',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const BinaryConverterTools = [
    {
        id:1,
        name: 'Text To Binary',
        href: '/texttobinaryconverter',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Binary To Text',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Binary To HEX',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Hex To Binary',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Binary To ASCII',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'ASCII To Binary',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Binary To Decimal',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Decimal To Binary',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Text To ASCII',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Decimal To HEX',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const OnlineCalculatorsTools = [
    {
        id:1,
        name: 'Age Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Percentage Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Average Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Confidence Interval Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Sales Tax Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Margin Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Probability Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Paypal Fee Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Discount Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Earnings Per Share Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'CPM Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Loan To Value Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'GST Calculator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

const OtherTools = [
    {
        id:1,
        name: 'Roman Numerals Date Converter',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    },
    {
        id:1,
        name: 'Credit Card Validator',
        href: '#',
        imageSrc: Plagiarism_Checker,
        imageAlt: 'No-Image',
    }
]

function Home() {
    return (
        <React.Fragment>
            <div className="container pt-4">
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-0 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="TextAnalysisTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Text Analysis Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Craft awe-inspiring logos, posters, invitations, flyers, and resume with 1000+ premade templates offered with advanced designing tools on our DesignStudio</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {TextAnalysisTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="DesignStudioTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Design Studio</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Craft awe-inspiring logos, posters, invitations, flyers, and resume with 1000+ premade templates offered with advanced designing tools on our DesignStudio</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {DesignStudioTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="ImagesEditingTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Images Editing Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Create a favicon, compress an image or resize a picture with a single click. All essentials for image editing are available in one place.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {ImagesEditingTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="KeywordsTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Keywords Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">For webmasters and SEO professionals, free, powerful and efficient keyword tools that provide you with thorough keyword research and analysis.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {KeywordsTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="BackLinkTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">BackLink Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">A set of backlink analysis tool to give you a comprehensive inside view of your link profile.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {BackLinkTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="WebsiteManagementTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Website Management Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">If you are struggling to get more traffic and enhance your website performance, then use these website management tools and in-depth web analysis.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {WebsiteManagementTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="WebsiteTrackingTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Website Tracking Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">A list of free tools in one place to measure, monitor, and keep track of your website’s performance.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {WebsiteTrackingTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="ProxyTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Proxy Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Use proxy tools to know your IP location or to get a free daily proxy list</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {ProxyTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="DomainsTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Domains Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">A range of domain related tools to find out domain age, domain authority, DNS records or expired domains, etc.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {DomainsTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="MetaTagsTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Meta Tags Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Create new meta tags or analyze the existing ones to get an in-depth analysis of your meta tags and web pages.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {MetaTagsTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="PasswordManagementTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Password Management Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Create a secure password, check password strength or encrypt a password; all tools related to password management are readily available.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {PasswordManagementTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="OnlinePDFTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Online PDF Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">All the tools you need to work with PDF files, at your fingertips. Merge, rotate, unlock, lock, watermark and convert PDFs with just a few clicks.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {OnlinePDFTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="DevelopmentTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Development Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Developer tools (or "development tools" or short "DevTools") are programs that allow a developer to create, test and debug software. Current browsers provide integrated developer tools, which allow to inspect a website.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {DevelopmentTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="UnitConverterTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Unit Converter Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Unit Converter is a measurement conversion tool that enables you to convert from over 1,000 different measurement units of Area, Basal Area (Forestry), Circular, Circular Area, dbh (Forestry), Energy/Work, Flow, Force, Length, Power, Pressure/Stress, Temperature, Time, Velocity, Volume, and Weight/Mass, for a total of over 100,000 possible conversions.  Unit Converter also enables you to add your own units and measurement classes to customize this software to your own needs.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {UnitConverterTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="BinaryConverterTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Binary Converter Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">Binary Converter is a tool used for various applications in digital electronics to perform binary to decimal conversion and decimal to binary conversion. In digital computation, the binary numbers are the numeric codes to represent the decimal numbers.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {BinaryConverterTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="OnlineCalculatorsTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Online Calculators Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">In turn, an online calculator has a different purpose: it is an interactive software tool that is designed to process a predefined set of mathematical functions in order to simulate a business process. Rather than the end user defining the operation, the tool’s designer is responsible for generating the calculation model.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {OnlineCalculatorsTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-white">
                    <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8" id="OtherTools">
                        <h2 className="font-bold tracking-tight text-gray-900 text-center text-5xl font-sans">Other Tools</h2>
                        <p className="text-2xl text-gray-900 text-center pt-3 pb-5 font-sans">SEO tools provide data and alerts about the overall health and success of your website.</p>
                        <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-5 xl:gap-x-8">
                            {OtherTools.map((product) => (
                                <div key={product.name} className="group relative flex flex-col items-center">
                                    <div className="aspect-w-1 aspect-h-1 overflow-hidden rounded-md group-hover:opacity-75 lg:aspect-none flex justify-center shadow-2xl lg:w-2/5 p-3">
                                        <img src={product.imageSrc} alt={product.imageAlt} className="h-full w-full object-cover object-center lg:w-full" />
                                    </div>
                                    <div className="mt-4 flex justify-center">
                                        <h3 className="text-sm text-gray-700 text-xl font-sans text-center">
                                            <Link to={product.href}>
                                                <span aria-hidden="true" className="absolute inset-0" />
                                                {product.name}
                                            </Link>
                                        </h3>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
            </div>
        </React.Fragment>
    )
}

export default Home;